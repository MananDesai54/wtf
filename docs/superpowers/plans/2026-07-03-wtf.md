...
**Spec:** `docs/superpowers/specs/2026-07-03-wtf-design.md`
...
**Interfaces:**
- Produces: `CAPTURE_SCRIPT: string` — injected into every page via `addInitScript`. Sends JSON payloads through `window.__wtf(json)` binding: `{type:'click', selector, label, bbox, pageUrl, timestamp}` and `{type:'spa-nav', url, timestamp}`.
...
- Produces: `wtf record --url <url> [--out <dir>] [--profile <dir>] [--viewport WxH]` and `wtf export <sessionDir> [--out <file>]`.
...
program
  ...
  .action(async (sessionDir: string, opts: { out: string }) => {
    ...
    console.log(`Wrote ${opts.out} — import it with the wtf Figma plugin.`);
  });
...
Run:
```bash
npm run build --workspace @wtf/cli
...
```bash
git add -A && git commit -m "feat(cli): wtf record/export commands"
```
...
Run:
```bash
npm run build --workspace @wtf/figma-plugin
npx vitest run
```
...
```bash
node packages/cli/dist/index.js record --url https://example.com --out /tmp/wtf-e2e
```
In the opened browser: click the "More information..." link. In the terminal: type a note, then `done`.
Expected: `/tmp/wtf-e2e/graph.json` has 2 nodes, 1 edge; 2 PNGs in `shots/`.

- [ ] **Step 2: Export**

```bash
node packages/cli/dist/index.js export /tmp/wtf-e2e --out /tmp/figma-import.json
```
Expected: file written, contains base64 images.

- [ ] **Step 3: Import in Figma**

In the Figma desktop app: any Design file → Plugins → Development → **Import plugin from manifest…** → select `packages/figma-plugin/manifest.json` → run "wtf importer" → choose `/tmp/figma-import.json`.
